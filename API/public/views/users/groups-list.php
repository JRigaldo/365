...
if( isset( $datas->tableDatas ) )
{
    ...
    {
        ?>
        <tr data-level="<?php echo $n; ?>" class="<?php echo (  $datas->response['updateid'] === $data->IdGroup ) ? ' success' : ''; ?>">  

            <?php self::_render( 'components/table-cell', [ 'content'=>( $n + 1 ) ] ); ?>

            <?php self::_render( 'components/table-cell', [ 'content'=>$data->NameGroup ] ); ?>
                            ...
                            foreach( $heading[ 'menus' ] as $menu )
                            {
                                ...
                                <tr> 
                                    ...
                                    if( isset( $menu->rights ) )
                                    {
                                        ...
                                        {
                                            self::_render( 'components/table-cell', [ 'urlajax'=>'users/groupactiveright/'.$rSymbol.'-'.$menu->IdMenu.'-'.$data->IdGroup, 'action'=>'active', 'state' => ( ( $isActive ) ? 1 : 0 ) ] );
                                        }
                                    }
                                    ?>
                                </tr>
            ...
            <?php self::_render( 'components/table-cell', [ 'url'=>'users/groupform/'.$data->IdGroup, 'action'=>'update', 'right'=>'update', 'rightaction' => '' ] ); ?>

            <?php self::_render( 'components/table-cell', [ 'urlajax'=>'users/groupdelete/'.$data->IdGroup, 'action'=>'delete', 'right'=>'delete', 'rightaction' => '', 'window-modal' => 'delete' ] ); ?>
        </tr>
        <?php
    }
}
